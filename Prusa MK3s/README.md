...
**"Set Filament Type"** - This macro asks what filament you going to use; PLA, PETg, ABS, or PC. This information will be used to change the "Heat Nozzle" macro, which can be then ran to heat the hotend for the specified filament.  The "Heat Nozzle" is also used, required,  by the macro "Filament Handling"  
**"Filament Handling"** - For any and all filament unloading, loading, and changing - whether during a print or otherwise, use this macro. This macro will load, unload, and change filament based on two conditions; whether it detects filament is current loaded or not, and if a print is in progress or not. Please note that since the logic functions look at the current status of the filament sensor- if your printer is currently empty and you intend to load filament, please do not place the filament in to the extruder until requested to do so by the macro. Else the macro will think filament is loaded and that you want to unload instead of load.  
